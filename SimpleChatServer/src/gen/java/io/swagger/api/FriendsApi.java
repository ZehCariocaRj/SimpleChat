...
@ApiParam(value = "Authentication token" ,required=true)@HeaderParam("Token") String token,@Context SecurityContext securityContext)
    ...
    public Response addFriend(@ApiParam(value = "User ID of person to friend",required=true) @QueryParam("targetId") Integer targetId
,
@ApiParam(value = "Authentication token" ,required=true)@HeaderParam("Token") String token,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.addFriend(targetId,token,securityContext);
    }
    @DELETE
